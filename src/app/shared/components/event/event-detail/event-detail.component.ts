...
export class EventDetailComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getEventById(eventId: string): void {
    ...
    this.eventService
      ...
      .then(() => {
        if (this.isLogged()) {
          this.commentService
            .getCommentsByEventId(this.event.id)
            .then((res) => {
              if (res && res.length > 0) {
                this.comments = res;
              }
    ...
    if (!this.authService.isLogged()) {
      ...
      if (this.event.isFavorite) {
        this.favoriteService
          .removeToFavorties(this.event.id)
          .then(() => {
            this.event.isFavorite = false;
            this.modalResultService.showModal(
              'Eliminat dels favorits',
              "S'ha eliminat correctament dels teus favorits",
              ModalResultIcon.success
            );
          })
          .catch((err) => {
            if (err) {
              this.modalResultService.showModal(
                'Error al eliminar',
                "S'ha produït un error al eliminar el favorit",
                ModalResultIcon.error
              );
            }
          });
      } else {
        this.favoriteService
          ...
          .then(() => {
            ...
            this.modalResultService.showModal(
              'Afegit als favorits',
              "S'ha afegit correctament als teus favorits",
              ModalResultIcon.success
            );
          })
          .catch((err) => {
            if (err) {
              this.modalResultService.showModal(
                'Error al afegir',
                "S'ha produït un error al afegir el favorit",
                ModalResultIcon.error
              );
            }
          });
      ...
      if (!this.event.isUserSubscribed) {
        this.eventService
          ...
          .then(() => {
            ...
            this.modalResultService.showModal(
              'Te has subscrit',
              "La teva subscripció s'ha guardat correctament!",
              ModalResultIcon.success
            );
          })
          .catch((err) => {
            if (err) {
              this.modalResultService.showModal(
                "No se t'ha pogut subscriure",
                "La teva subscripció no s'ha guardat correctament!",
                ModalResultIcon.error
              );
            }
          });
        ...
        this.eventService
          ...
          .then(() => {
            ...
            this.modalResultService.showModal(
              'Te has donat de baixa',
              "Se t'ha donat de baixa de la teva subscripció!",
              ModalResultIcon.success
            );
          })
          .catch((err) => {
            if (err) {
              this.modalResultService.showModal(
                "No se t'ha pogut desubscriure",
                "No se t'ha pogut donar de baixa de la teva subscripció!",
                ModalResultIcon.error
              );
            }
          });
  ...
  sendComment(): void {
    if (this.isLogged()) {
      ...
      Swal.fire({
        ...
        this.commentService
          .sendComment(this.event.id, comment)
          .then((res) => {
          ...
          .catch((err) => {
            const title = 'Acte ja comentat!';
            const text = 'Ja has fet un comentari sobre aquest acte';
  ...
  rateIt(): void {
    if (this.isLogged()) {
      ...
      Swal.fire({
        ...
        this.commentService
          .addRating(this.event.id, this.event.rating.rating)
          .then(() => {
            const title: string = 'Puntuació enviada';
            const text: string =
              "S'ha registrat correctament la teva puntuació!";
  ...
  isTheDayOfEvent(): boolean {
    const eventStartDate = UtilsService.humanitizeEpochDate(
    ...
    return (
      this.event.startDate && eventStartDate <= today && today <= eventEndDate
    );
  }
  ...
  goToCategoryResult(event: Event): void {
    ...
    const redirectUrl: string = this.isLogged()
      ? 'event/dashboard/event/search'
      : '/search';
