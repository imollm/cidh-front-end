...
export class EventCreateEditComponent implements OnInit, AfterViewChecked {
  ...
  labels: Label[] = [];
  selectedLabels = [];
  private readonly URLRegex: string =
  ...
  private initLabelCheckboxes(): void {
    if (this.mode === 'create') {
      this.labels.forEach(() =>
        this.labelIdsFormArray.push(new FormControl(false))
      );
  ...
  private prepareLabelIdsToBeSendIt(): void {
    this.form.value.labelIds = this.form.value.labelIds
      .map((checked, i) =>
        checked && i < this.labels.length ? this.labels[i].id : null
      )
      .filter((v) => v !== null);
  }
  ...
  private redirectUser(): void {
    this.router
      .navigate(['/profile/dashboard/events/list'])
      .then(() => {
  ...
  onSubmit(): void {
    ...
    if (this.form.valid) {
      ...
      if (this.mode && this.mode === 'create') {
        ...
        this.eventService
          .updateEvent(this.eventId, this.form.value)
          .then((res) => {
            this.event = res;
          })
          .then(() => this.redirectUser())
